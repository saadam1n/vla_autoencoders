...
class TransformerAutoencoderMHA(nn.Module):
    ...
    def __init__(self, num_actions, token_dimension, num_embeddings, num_input_tokens, num_output_tokens):
        ...
        self.decoders = nn.ModuleList([
            DoubleLayerMLP((i + 1) * self.token_dimension, self.flattened_input_dim, expansion_ratio=2) for i in range(self.num_output_tokens)
        ])
    ...
    def forward(self, x : torch.Tensor):
        ...
        reconstructed = torch.stack([
            self.decoders[i](output_tokens[:, :(i + 1) * self.token_dimension]).view_as(x) for i in range(self.num_output_tokens)
        ], dim=1)
